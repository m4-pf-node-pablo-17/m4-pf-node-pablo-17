...
describe('/products', () => {
    ...
    test('POST /products - Should be able to create a product', async () => {
        ...
        const userLoginResponse = await request(app).post('/login').send(mockedLoginProductUser)
        const token = `Bearer ${userLoginResponse.body.tokenUser}`

        const response = await request(app).post('/products').set('Authorization', token).send(mockedProductRequest)
    ...
    test('POST /products - Should not be able to create product with invalid body', async () => {
        ...
        const response = await request(app).post('/products').set('Authorization', token).send(mockedProductInvalidRequest)
    ...
    test('POST /products - Should not be able to create a product without authentication', async () =>{
        const response = await request(app).post('/products').send(mockedProductRequest)
    ...
    test('GET /products - Must be able to list all products', async () => {
        ...
        const response = await request(app).get('/products').set('Authorization', token)   
    ...
    test('GET /products/:id - Must be able to list one product', async () => {
        ...
        await request(app).post('/products').set('Authorization', token).send(mockedProductRequest)
        const product = await request(app).get('/products').set('Authorization', token)
        const response = await request(app).get(`/products/${product.body[0].id}`).set('Authorization', token)
    ...
    test('GET /products/:id - Should not be able to list one product without authentication', async () =>{
        ...
        await request(app).post('/products').set('Authorization', token).send(mockedProductRequest)
        const product = await request(app).get('/products').set('Authorization', token)
        const response = await request(app).get(`/products/${product.body[0].id}`)
    ...
    test('GET /products/users/:id - Must be able to list all products by user', async () => {
        ...
        await request(app).post('/products').set('Authorization', token).send(mockedProductRequest)
        const user = await request(app).get('/users').set('Authorization', token) 
        const response = await request(app).get(`/products/users/${user.body[0].id}`).set('Authorization', token)   
    ...
    test('GET /products/users/:id - Must not be able to list all products by user without authentication', async () => {
        ...
        await request(app).post('/products').set('Authorization', token).send(mockedProductRequest)
        const user = await request(app).get('/users').set('Authorization', token) 
        const response = await request(app).get(`/products/users/${user.body[0].id}`)
    ...
    test('PATCH - /products/:id - Should be able to update a product', async () => {
        ...
        expect(productUpdated.body.price).toEqual(207.50)
    })
    ...
    test('PATCH - /products/:id - Should not be able to update a product without authentication', async () => {
        ...
        const response = await request(app).patch(`/products/${productTobeUpdateRequest.body[0].id}`).send(updatedProductValues)
    ...
    test('PATCH - /products/:id - Should not be able to update a product with invalid id', async () => {
        ...
        const response = await request(app).patch('/products/1bde224f-5ad7-48c3-3d27-8e383591baaa').set('Authorization', token).send(updatedProductValues)
    ...
    test('DELETE /products/:id -  Must not be able to soft delete product without authentication', async () => {
        ...
        const response = await request(app).delete(`/products/${productTobeDeletedRequest.body[0].id}`)
    ...
    test('DELETE /products/:id -  Must not be able to soft delete product with invalid id', async () => {
        ...
        const response = await request(app).delete('/products/1bde224f-5ad7-48c3-3d27-8e383591baaa').set('Authorization', token)
