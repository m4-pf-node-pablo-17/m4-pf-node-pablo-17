...
import { app } from '../../../app';
import { mockedCommentRequest, mockedLoginCommentUser, updatedComment } from '../../mocks/integration/comments.mocks';
...
describe('/message', () => {
    ...
    /* test("PATCH /message/:id - Should be able to update a message", async () => {
        await request(app).post('/users').send(mockedCreateUser)
        const userLoginResponse = await request(app)
        .post('/login')
        .send(mockedLoginCommentUser)
        ...
        const commentTobeUpdateId = commentTobeUpdateRequest

        const response = await request(app)
            .patch(`/message/${commentTobeUpdateId}/posts`)
            .set("Authorization", token)
            .send(updatedComment)
            console.log(response.body);
            

        const messageUpdated = await request(app)
            .get(`/message/${post.body.id}`)
            .set("Authorization", token)

        expect(response.status).toBe(200)
        expect(messageUpdated.body[0].text).toEqual("olá, tudo bem?")
    }) */
    ...
    test("DELETE /message/:id -  Must be able to delete message", async () => {
        ...
        const response = await request(app)
            .delete(`/message/${messageTobeDeletedRequest}`)
            .set("Authorization", token)
